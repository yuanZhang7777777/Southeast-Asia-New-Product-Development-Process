...
def seed_archive_rows() -> None:
    ...
    with SessionLocal() as db:
        ...
        # 主销售员有值但无是否认领/单销/理由 → 不认领，保留来源缺失原因
...
def test_apply_creates_claim_rows_without_tasks_or_notifications() -> None:
    ...
    with SessionLocal() as db:
        ...
        assert by_sub["A428-AMBI"].claim_result == "reject"
        assert by_sub["A428-AMBI"].reject_reason == "来源表未填写认领单销"
        assert by_sub["B526-ZERO"].claim_result == "reject"
        assert by_sub["B526-ZERO"].reject_reason == "来源表认领单销为0"
        ...
        assert audit_entry.detail["created"] == 8
...
def test_cleaned_0414_uses_standard_bz_to_ce_claim_columns() -> None:
    with SessionLocal() as db:
        _add_opportunity(
            db,
            sheet="开发0414期",
    ...
    with SessionLocal() as db:
        claim = db.query(models.SalesClaimForecast).filter_by(source_column=CLAIM_SOURCE_COLUMN).one()
        assert claim.salesperson_name == "标准销售"
        assert claim.claim_result == "claim"
        assert claim.claim_daily_sales == 1
        assert claim.feedback_summary == "标准反馈"


def test_caigen_split_period_uses_standard_bz_to_ce_claim_columns() -> None:
    with SessionLocal() as db:
        _add_opportunity(
            db,
            sheet="开发0708期-财根",
    ...
    assert report["created"] == 1
    assert report["by_period"]["开发0708期-财根"]["reject"] == 1
    with SessionLocal() as db:
        claim = db.query(models.SalesClaimForecast).filter_by(source_column=CLAIM_SOURCE_COLUMN).one()
        assert claim.salesperson_name == "财根销售"
        assert claim.claim_result == "reject"
        assert claim.reject_reason == "利润不足"


def test_caigen_split_period_accepts_shifted_claim_values_under_standard_headers() -> None:
    with SessionLocal() as db:
        _add_opportunity(
            db,
            sheet="开发0701期-财根",
            sub_sku="CG-0701-CLAIM",
            source_row=25,
            cells=_cells(
                [
                    ("BZ", "不认领理由", "赵钰婷"),
    ...
    assert report["claim"] == 1
    assert report["reject"] == 1
    with SessionLocal() as db:
        claims = db.query(models.SalesClaimForecast).filter_by(source_column=CLAIM_SOURCE_COLUMN).all()
        by_seller = {claim.salesperson_name: claim for claim in claims}
        assert by_seller["赵钰婷"].claim_result == "claim"
        assert by_seller["赵钰婷"].claim_daily_sales == 1
        assert by_seller["冯卓宏"].claim_result == "reject"
        assert by_seller["冯卓宏"].reject_reason == "利润不足"
...
def test_backfill_can_limit_to_the_confirmed_period_scope() -> None:
    with SessionLocal() as db:
        _add_opportunity(
            db,
            sheet="开发0727期-财根",
            sub_sku="CAIGEN-SKIP",
            source_row=31,
            cells=_cells([("CM", "销售员1", "销售甲"), ("CN", "预估单销", 1)]),
        )
        db.commit()

    with SessionLocal() as db:
        report = backfill_selection1_claims(db, periods=CANONICAL_CLAIM_PERIODS)
...
def test_rerun_is_idempotent() -> None:
    ...
    assert second["zero_daily_sales"] == 0
    assert second["unclaimed"] == 0
    with SessionLocal() as db:
        assert db.query(models.SalesClaimForecast).count() == 8
...
def test_normalize_can_limit_to_the_confirmed_period_scope() -> None:
    with SessionLocal() as db:
        _add_opportunity(
            db,
            sheet="开发0727期-财根",
            sub_sku="CAIGEN-STALE",
            source_row=31,
            cells=_cells([("CM", "销售员1", "销售甲"), ("CN", "预估单销", 1)]),
        )
        db.flush()
        opportunity = db.query(models.NewProductOpportunity).filter_by(sub_sku="CAIGEN-STALE").one()
        db.add(
            models.SalesClaimForecast(
                opportunity_id=opportunity.id,
                salesperson_name="历史错误销售",
    ...
    with SessionLocal() as db:
        report = normalize_selection1_claims(db, apply=True, periods=CANONICAL_CLAIM_PERIODS)
        db.commit()

    assert report == {"updated": 0, "deleted_extra": 0, "skipped_referenced": 0}
    with SessionLocal() as db:
        claim = db.query(models.SalesClaimForecast).filter_by(source_column=CLAIM_SOURCE_COLUMN).one()
        assert claim.salesperson_name == "历史错误销售"
...
def test_revert_removes_only_unreferenced_backfill_rows() -> None:
    ...
    with SessionLocal() as db:
        ...
        db.commit()
    assert report == {"reverted_claims": 8, "skipped_referenced": 0}
    with SessionLocal() as db:
        # platform 行保留，本模块行清零
        assert db.query(models.SalesClaimForecast).count() == 1
        assert db.query(models.SalesClaimForecast).filter_by(source_column=CLAIM_SOURCE_COLUMN).count() == 0
...
def test_revert_skips_rows_referenced_by_arrival_record() -> None:
    ...
    with SessionLocal() as db:
        report = revert_selection1_claims(db, actor="test")
        db.commit()
    assert report == {"reverted_claims": 7, "skipped_referenced": 1}
    with SessionLocal() as db:
