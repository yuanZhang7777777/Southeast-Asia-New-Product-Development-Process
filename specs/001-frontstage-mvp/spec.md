...
主管对导入后的主 SKU 组执行一键分配，系统按重点站点硬过滤并以当前主 SKU 组负载优先给出建议；重点品类、人员优先级和站点内顺序只处理同负载并列，主管确认后生成运营认领任务。
...
2. **Given** 运营人员配置包含重点站点、重点品类和优先级，**When** 系统生成分配建议，**Then** 先按重点站点硬过滤，再优先选择当前未完成主 SKU 组负载更少的人；负载相同才按重点品类命中、人员优先级和站点内顺序决胜。
