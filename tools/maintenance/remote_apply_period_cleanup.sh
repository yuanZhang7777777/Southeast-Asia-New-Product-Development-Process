...
backup_dir="backups/period_cleanup_${stamp}"
mkdir -p "$backup_dir"
...
docker compose cp "$TMP_DIR/cleanup_plm_uat_and_history_status.py" api:/tmp/cleanup_plm_uat_and_history_status.py >/dev/null
echo "apply cleanup"
docker compose exec -T -e PYTHONPATH=/app api python /tmp/cleanup_plm_uat_and_history_status.py --apply | tee "$backup_dir/apply_cleanup.json"
