...
统一开发分支为 `lxc/integrated-workflow`；开发环境当前部署 `36a9ae9`，访问地址为 [http://139.224.2.166:18081](http://139.224.2.166:18081)。开发库已迁移到 `d0e2f3a4b567 (head)`；该版本同时提供备货申请/导出工作台和可折叠的“主 SKU → Item → 周记录”观察工作台。生产环境仍运行 `b373d65`，本次未连接或变更生产，不得把开发部署视为生产发布。

当前工作区另有尚未部署的 UAT 易用性候选：运营改填长 / 宽 / 高（cm）后由前后端自动计算单个体积，离开申请编辑区自动保存；备货条目按主 / 子 SKU 固定排序，主管复核展开右栏时左侧不再拉伸或叠加通用侧栏。源码 Alembic head 为 `e1f2a3b4c678`，开发运行库仍为 `d0e2f3a4b567`。
