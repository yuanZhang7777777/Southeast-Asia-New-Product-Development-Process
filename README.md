...
东南亚海外仓新品流程平台。前段已覆盖两张内部反馈表导入、主管分配、运营认领/不认领、主管复核、备货申请表和中央字段追溯表导出；后段开发环境已覆盖到货后二次调研、刊登与 Item 独立周期观察。本地候选进一步补齐销售自选三分支、运营备货申请保存/提交、ERP 体积查询失败时手填降级，以及主管勾选已提交申请导出。
...
统一开发分支为 `lxc/integrated-workflow`；开发环境当前部署 `c017c5d`，访问地址为 [http://139.224.2.166:18081](http://139.224.2.166:18081)。该版本已采用可折叠的“主 SKU → Item → 周记录”工作台，并在商品详情提供二次调研、刊登观察只读历史。生产环境仍运行 `b373d65`，不得把开发部署视为生产发布。

销售自选与新版备货申请目前只是本地候选，尚未部署到开发环境。主管复核先按“销售员 + 子 SKU”生成草稿，运营填写并提交后才进入主管可勾选导出范围；真实导出后才进入 `waiting_arrival` 并允许 PLM 按导出快照国家精确关联。
