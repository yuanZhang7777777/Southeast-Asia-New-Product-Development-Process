...
当前生产入口为 `http://101.132.26.138:8080/`。开发环境通过 SSH 别名 `hz-new-product-dev` 管理，公网端口为 `18081`，但已在 2026-08-04 按用户要求停止 Compose 服务；需要恢复时必须先加锁再启动。开发与生产 Alembic 当前均为 `a7b8c9d0e123 (head)`，已包含选品2标准导入、公共选品池多人加入、独立认领/复核、专属详情分区和二次调研来源模块，以及一次性历史选品2、选品3/4只读迁移。历史选品3/4按选品1档案展示但不进入公共池；生产已于 2026-07-31 完成备份、dry-run、幂等导入和读回，自动发卡与 PLM 同步继续关闭。
