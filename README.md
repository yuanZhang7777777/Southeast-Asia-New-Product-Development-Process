...
统一开发分支为 `lxc/integrated-workflow`；开发环境当前部署 `af4addc3f765`，访问地址为 [http://139.224.2.166:18081](http://139.224.2.166:18081)。开发库保持 `e1f2a3b4c678 (head)`；既有备货申请和紧凑观察工作台不变，本次只新增历史关注清单、开发专用 PLM 到货试运行和 FineBI 多周期历史 Item 候选对账。生产环境仍运行 `b373d65`，本次未连接或变更生产，不得把开发部署视为生产发布。
