...
7. 写淘汰提醒状态变化回归：利润款 -> 淘汰款 -> 淘汰款 -> 稳定款 -> 淘汰款只生成两次主管提醒事件，失败可重试、成功不重复。
8. 写停止 / 恢复、首轮完成和手动新增后续周期测试。
9. 最小实现 Schemas、服务和路由，所有 actor 从认证上下文取得并写 AuditLog。
10. 运行 `pytest backend/tests/test_listing_observations.py -q`。
...
1. 增加按主 SKU、国家、负责人读取刊登 / 周期历史的后端测试。
2. 在商品详情增加只读区，复用工作台响应类型和格式化函数，不建设第二套编辑器。
3. 运行相关后端测试和前端构建。
