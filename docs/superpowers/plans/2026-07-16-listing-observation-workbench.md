...
5. 写列表与权限测试：运营强制本人、主管查看全部并筛选；刊登记录更新不接受负责人变更，不实现转交流程。
6. 写周期复盘测试：定位和优化必填，第 4 周总结必填，整批原子；第 1 周定位默认来自二次调研，后续周期默认使用最近历史定位并在缺失时回退到二次调研。
7. 写已完成周期再次编辑测试：负责人运营和主管可修改人工填写项，其他运营越权失败，自动指标不变、状态仍为已完成并记录修改审计，且不重新生成运营待复盘提醒。
8. 写定位与跟踪解耦回归：第 N 周提交淘汰款后 Item 仍正常跟踪，第 N+1 周继续取数和复盘并可改回其他定位；第 4 周淘汰款仍要求总结。
9. 写淘汰提醒状态变化回归：首次提交或修改已完成周期时，利润款 -> 淘汰款 -> 淘汰款 -> 稳定款 -> 淘汰款只生成两次主管提醒事件，失败可重试、成功不重复。
10. 写停止 / 恢复、首轮完成和手动新增后续周期测试；覆盖“成功取数后停止跟踪，已有待复盘周期仍可提交，但后续周期生成、后续取数和后续提醒均被阻止”。
11. 最小实现 Schemas、服务和路由，所有 actor 从认证上下文取得并写 AuditLog。
12. 运行 `pytest backend/tests/test_listing_observations.py -q`。
...
1. 先写失败测试：无源记录保持待取数，真实零进入待复盘，非零毛利率计算正确；首次成功后再次传入不同指标也不得覆盖原值。
2. 实现 `apply_week_metrics`：只在 `metrics_fetched_at` 为空时按 Item + 周期写入首次有效记录并保留来源追溯，后续调用直接返回已有记录。
...
1. 先写失败测试：四个刊登必填、trim 后批内重复、组合筛选、百分比格式、无数据与真实 0、服务端行错误映射，以及按用户 / 任务键恢复和清除浏览器草稿。
2. 实现最小纯函数 helper；浏览器草稿直接复用原生 `localStorage`，不增加依赖或服务端契约。
...
3. 实现单张全量表和业务状态、常用 / 高级筛选；默认筛选待刊登，不增加顶部状态分栏，也不把内部待取数作为运营筛选项。
4. 实现原生横向 table、sticky 关键列、同主 SKU 分组、表内定位 / 优化编辑；同一 Item 的历史周期在同一表内按周次连续展示，已完成周期提供重新编辑入口并复用同一校验和提交逻辑。
