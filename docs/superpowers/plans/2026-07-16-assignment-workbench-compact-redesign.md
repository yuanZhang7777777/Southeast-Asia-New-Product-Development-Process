...
- 不新增依赖、不改权限；2026-07-16 公平性反馈后，允许把现有推荐排序局部改为负载优先，不引入新的求解器。
...
约束“运营配置”必须紧邻“提交分配”；负载容器必须吸顶并以自适应网格完整换行，不出现内部横向滚动；同站点拖拽必须改变 `display_order`，跨站点拖拽必须保持原样。

- [x] **Step 2: 调整命令栏与运营负载**

把配置入口移到“提交分配”旁并组成不可拆分操作组；命令栏按实际空间换行；负载卡改为吸顶自适应网格并继续使用现有实时预览计数。
...
`npm test` 通过 41 项，`npm run build` 通过；Playwright/Edge 最新验证 2048px、1366px 下 16 名运营分别为 2 行、4 行，负载区和整页横向溢出为 0，相邻操作组间距 8px；此前 390px、吸顶、实时计数和真实拖拽检查继续通过。

### Task 6: 负载优先公平分配跟进

**Files:**
- Modify: `backend/app/assignment_rules.py`
- Modify: `backend/tests/test_assignment_rules.py`
- Modify: `frontend/src/App.tsx`
- Modify: `frontend/src/styles.css`
- Modify: `frontend/tests/reviewLayout.test.ts`

- [x] **Step 1: 复现高优先级人员独占同类任务**

旧排序把类目匹配放在负载之前，7 个同类主 SKU 会全部分给唯一类目专员。

- [x] **Step 2: 改为负载优先列表调度**

站点硬过滤后先比较当前主 SKU 组负载；只有负载相同才比较类目、人员优先级、自定义顺序和姓名。7 组、3 人的回归结果为 `3/2/2`。

- [x] **Step 3: 完整验证**

后端 `pytest -q` 通过 194 项；前端 `npm test` 通过 41 项，`npm run build` 通过；Playwright/Edge 通过上述 2048px/1366px 验收。
