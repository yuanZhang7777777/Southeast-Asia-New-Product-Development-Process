...
- [x] **Step 3: 用 `Intl.NumberFormat` 实现并接入价格、月销、单销显示**
...
- [x] **Step 4: 在当前主管编辑面板增加状态和原生折叠参数区**
...
- [x] **Step 3: 复用 `submit_review` 实现单事务批量接口**

通过动作按所选状态映射到 `approved` 或 `confirmed_not_claim`；拒绝映射到 `returned_for_supplement` 并要求原因。

- [x] **Step 4: 增加两段筛选、全选、批量通过和批量拒绝**

切换筛选时清空选择，后端仍校验同类型。

- [x] **Step 5: 运行全量验证并检查差异**
